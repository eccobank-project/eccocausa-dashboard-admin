-- Script para limpiar y recrear las funciones RPC de sectores
-- Ejecutar este SQL en el SQL Editor de Supabase

-- 1. Eliminar todas las funciones existentes relacionadas con sectores
DROP FUNCTION IF EXISTS create_sector_with_geom;
DROP FUNCTION IF EXISTS update_sector_with_geom;
DROP FUNCTION IF EXISTS get_sector_with_geom;
DROP FUNCTION IF EXISTS get_all_sectors_with_geom;

-- 2. Crear función para crear sectores con geometría
CREATE OR REPLACE FUNCTION create_sector_with_geom(
    p_nombre TEXT,
    p_id_ciudad INTEGER,
    p_color TEXT,
    p_dia_recojo BIGINT,
...
-- 3. Crear función para actualizar sectores con geometría
CREATE OR REPLACE FUNCTION update_sector_with_geom(
    p_id INTEGER,
...
-- 4. Crear función para obtener un sector con geometría
CREATE OR REPLACE FUNCTION get_sector_with_geom(p_sector_id INTEGER)
RETURNS TABLE(
    id INTEGER,
    nombre TEXT,
    id_ciudad INTEGER,
...
-- 5. Crear función para obtener todos los sectores con geometría
CREATE OR REPLACE FUNCTION get_all_sectors_with_geom()
RETURNS TABLE(
    id INTEGER,
    nombre TEXT,
    id_ciudad INTEGER,
