-- Script para verificar y corregir los tipos de datos
-- Ejecutar este SQL en el SQL Editor de Supabase

-- 1. Verificar la estructura real de la tabla sector
SELECT 
    column_name, 
    data_type, 
    is_nullable,
...
-- 2. Eliminar las funciones actuales (todas las versiones posibles)
DROP FUNCTION IF EXISTS create_sector_with_geom(TEXT, INTEGER, TEXT, BIGINT, TEXT);
DROP FUNCTION IF EXISTS create_sector_with_geom(TEXT, BIGINT, TEXT, BIGINT, TEXT);
DROP FUNCTION IF EXISTS update_sector_with_geom(INTEGER, TEXT, INTEGER, TEXT, BIGINT, TEXT);
DROP FUNCTION IF EXISTS update_sector_with_geom(BIGINT, TEXT, BIGINT, TEXT, BIGINT, TEXT);
DROP FUNCTION IF EXISTS get_sector_with_geom(INTEGER);
DROP FUNCTION IF EXISTS get_sector_with_geom(BIGINT);
DROP FUNCTION IF EXISTS get_all_sectors_with_geom();

-- 3. Crear funciones con tipos corregidos (usando BIGINT para id)
...
-- 4. Crear función para actualizar sectores
CREATE FUNCTION update_sector_with_geom(
    p_id BIGINT,
    p_nombre TEXT DEFAULT NULL,
    p_id_ciudad BIGINT DEFAULT NULL,
...
BEGIN
    ...
    -- Retornar el sector actualizado
    RETURN QUERY
    SELECT 
        updated_sector.id,
...
-- 5. Crear función para obtener un sector
CREATE FUNCTION get_sector_with_geom(p_sector_id BIGINT)
RETURNS TABLE(
    id BIGINT,
    nombre TEXT,
    id_ciudad BIGINT,
...
-- 6. Crear función para obtener todos los sectores
...
-- 9. Test de la función (cambia los valores según tu estructura)
SELECT 'Test: Funciones creadas con tipos BIGINT' as status;